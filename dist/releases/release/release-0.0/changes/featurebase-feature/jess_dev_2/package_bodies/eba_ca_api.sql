...
-- changeset JESS_DEV_2:1765789861551 stripComments:false  logicalFilePath:featurebase-feature\jess_dev_2\package_bodies\eba_ca_api.sql
-- sqlcl_snapshot src/database/jess_dev_2/package_bodies/eba_ca_api.sql:null:2c0bd23520caae26504b8c065fa630e0c28147fb:create

create or replace package body eba_ca_api as
    ...
    begin
        insert into eba_ca_series (
            ...
            recur_freq
                   ...
                   p_end_date,
                   p_recur_freq ) returning series_id into l_series_id;
...
--         l_cnt := c1.end_date - c1.start_date; 
                l_cnt := trunc(to_number(substr((c1.end_date - c1.start_date),
                                                1,
                                                instr(c1.end_date - c1.start_date, ' '))));
...
--         l_cnt := trunc((c1.end_date - c1.start_date)/7); 
                l_cnt := trunc(to_number(substr((c1.end_date - c1.start_date),
                                                1,
                                                instr(c1.end_date - c1.start_date, ' '))) / 7);
...
--         l_cnt := trunc((c1.end_date - c1.start_date)/14); 
                l_cnt := trunc(to_number(substr((c1.end_date - c1.start_date),
                                                1,
                                                instr(c1.end_date - c1.start_date, ' '))) / 14);
                ...
                l_cnt := trunc(months_between(to_date(to_char(c1.end_date, 'DD-MON-RRRR'),
               'DD-MON-RRRR'),
                                              to_date(to_char(c1.start_date, 'DD-MON-RRRR'),
                                              'DD-MON-RRRR')));
    ...
    begin
        ...
        for c1 in (
            ...
            if c1.recur_freq = 'D' then
                l_cnt := trunc(to_number(substr((c1.end_date - l_begin_date),
                                                1,
                                                instr(c1.end_date - l_begin_date, ' '))));
                ...
                l_cnt := trunc(to_number(substr((c1.end_date - l_begin_date),
                                                1,
                                                instr(c1.end_date - l_begin_date, ' '))) / 7);
                ...
                l_cnt := trunc(to_number(substr((c1.end_date - l_begin_date),
                                                1,
                                                instr(c1.end_date - l_begin_date, ' '))) / 14);
                ...
                l_cnt := trunc(months_between(to_date(to_char(c1.end_date, 'DD-MON-RRRR'),
               'DD-MON-RRRR'),
                                              to_date(to_char(l_begin_date, 'DD-MON-RRRR'),
                                              'DD-MON-RRRR')));
    ...
    begin
      ...
      -- only for non-recurring events, recur pass in db value (update of recurring date/time handled elsewhere) 
            if to_char(c1.event_date_time, 'DD-MON-RRRR HH:MIam') != to_char(p_event_date_time, 'DD-MON-RRRR HH:MIam') then
                update eba_ca_events
                set
                    event_date_time = p_event_date_time
                where
                    event_id = p_event_id;
    ...
    begin
        update eba_ca_events
        set
            event_date_time = to_date(to_char(event_date_time, 'DD-MON-RRRR')
                                      || to_char(p_event_date_time, 'HH:MIam'),
        'DD-MON-RRRRHH:MIam')
        where
            event_id = p_event_id;
    ...
    begin
        for c1 in (
            ...
            from
                eba_ca_event_types
            where
                upper(p_event_type) = upper(type_name)
        ) loop
        ...
        if l_type_id is null then
            ...
            insert into eba_ca_event_types (
                type_name,
                display_color,
                border_color,
                internal_yn,
                color_pref_id
                       ...
                       l_border_color,
                       p_internal_yn,
                       p_color_pref_id ) returning type_id into l_type_id;
   ...
   -- 
    ...
    begin
        if p_new_event_type is not null then
            l_event_type_id := create_event_type(
                p_event_type    => p_new_event_type,
        ...
        if p_recur_flag = 'Y' then
            l_series_id := create_series_l(
                p_start_date => cast(trunc(to_date(to_char(p_event_date_time, 'DD-MON-RRRR'),
      'DD-MON-RRRR')) as timestamp),
            ...
            -- update series end date, if last event was deleted 
                        for c3 in (
                            ...
                            update_series_end_date_l(
                                ...
                                p_end_date  => cast(trunc(to_date(to_char(c3.last_date, 'DD-MON-RRRR'),
      'DD-MON-RRRR')) as timestamp)
                            );
                        end loop;
                    end if;
...
--                  if c2.event_date_time >= c1.event_date_time then 
                            if trunc(to_date(to_char(c2.event_date_time, 'DD-MON-RRRR'),
      'DD-MON-RRRR')) >= trunc(to_date(to_char(c1.event_date_time, 'DD-MON-RRRR'),
      'DD-MON-RRRR')) then
                                delete_event_l(p_event_id => c2.event_id);
                            end if;
                        end if;
            ...
            -- update series end date 
                        for c2 in (
                            ...
                            update_series_end_date_l(
                                p_series_id => c1.series_id,
                                p_end_date  => cast(trunc(to_date(to_char(c2.last_date, 'DD-MON-RRRR'),
      'DD-MON-RRRR')) as timestamp)
                            );
                        end loop;
                    end if;
   ...
   -- 
    ...
    begin
      ...
      -- simple update 
            if
                nvl(p_recur_flag, 'N') != 'Y'
                and c1.series_id is null
            then
                update_event_l(
      ...
      -- removing recurrence 
                ...
                p_recur_flag is null
                and c1.series_id is not null
            then
      ...
      -- updating recurring event 
         ...
         -- updating standard columns 
                if p_event_name != c1.event_name
                or ( nvl(p_type_id, 0) != nvl(c1.type_id, 0)
                or p_new_event_type is not null )
                or nvl(p_calendar_id, 0) != nvl(c1.calendar_id, 0)
                or nvl(p_event_desc, '0') != nvl(c1.event_desc, '0')
                or nvl(p_contact_person, '0') != nvl(c1.contact_person, '0')
                or nvl(p_contact_email, '0') != nvl(c1.contact_email, '0')
                or p_display_time != c1.display_time
                or nvl(p_location, '0') != nvl(c1.location, '0')
                or nvl(p_link_name_1, '0') != nvl(c1.link_name_1, '0')
                or nvl(p_link_url_1, '0') != nvl(c1.link_url_1, '0')
                or nvl(p_link_name_2, '0') != nvl(c1.link_name_2, '0')
                or nvl(p_link_url_2, '0') != nvl(c1.link_url_2, '0')
                or nvl(p_link_name_3, '0') != nvl(c1.link_name_3, '0')
                or nvl(p_link_url_3, '0') != nvl(c1.link_url_3, '0')
                or nvl(p_tags, '0') != nvl(c1.tags, '0')
                or p_duration != c1.duration then
      ...
      'DD-MON-RRRR')) as timestamp),
               ...
               -- start time change but no date change 
                            ...
                            to_char(p_event_date_time, 'HH:MIam') != to_char(c1.event_date_time, 'HH:MIam')
                            and to_char(p_event_date_time, 'DD-MON-RRRR') = to_char(c1.event_date_time, 'DD-MON-RRRR')
                        then
                            if p_update_request = 'O' then
                                update_start_time_l(
                                    p_event_id        => p_event_id,
                  ...
                  -- when new end date earlier 
                     ...
                     -- remove future events 
                                for c3 in (
                                    ...
                                    if trunc(to_date(to_char(c3.event_date_time, 'DD-MON-RRRR'),
      'DD-MON-RRRR')) > trunc(to_date(to_char(p_recur_end_date, 'DD-MON-RRRR'),
      'DD-MON-RRRR')) then
                                        delete_event_l(p_event_id => c3.event_id);
                                    end if;
                                end loop; 
            ...
            -- 
                ...
                if l_valid then
                    for c1 in (
                        ...
                        where
                                collection_name = 'BULK_MBRS_CREATE'
    ...
    begin
        for c1 in (
            select
                trunc(to_date(to_char(event_date_time, 'DD-MON-RRRR'),
                      'DD-MON-RRRR')) event_date
            from
                eba_ca_events
            where
                event_id = p_event_id
        ...
        for c1 in (
            ...
            from
                eba_ca_timeframes
            where
                    l_event_date >= start_date
                and l_event_date <= end_date
            order by
                start_date asc
        ) loop
            l_timeframes := l_timeframes
                            || ', '
                            || c1.tf_name;
        end loop;
