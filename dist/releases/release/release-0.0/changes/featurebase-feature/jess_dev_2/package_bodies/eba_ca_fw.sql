...
-- changeset JESS_DEV_2:1765789861889 stripComments:false  logicalFilePath:featurebase-feature\jess_dev_2\package_bodies\eba_ca_fw.sql
-- sqlcl_snapshot src/database/jess_dev_2/package_bodies/eba_ca_fw.sql:null:d1b030df76a3015f72956813e00af0e5f2fcc948:create

create or replace package body eba_ca_fw as
    ...
    function conv_txt_html (
        ...
        l_length       number;
    begin
        l_html_message := replace(l_html_message,
                                  chr(10),
                                  '<br />');
        l_html_message := replace(l_html_message,
                                  chr(13),
    ...
    function conv_urls_links (
        ...
        l_endofurl    varchar2(4000) default chr(10)
                                          || chr(13)
                                          || chr(9)
                                          || ' )<>';
        l_url         varchar2(4000);
    ...
    begin
        ...
        for i in 1..1000 loop
            ...
            exit when n = 0
            or length(l_string) > 32000;
            for j in 0..length(l_string) - n loop
                if ( instr(l_endofurl,
                           substr(l_string, n + j, 1)) > 0 ) then
                    l_url := rtrim(
                        substr(l_string, n, j),
                        '.'
                        || chr(32)
    ...
    begin
        ...
        for i in 1..l_merge_tags.count loop
            merge into eba_ca_tags_type_sum s
            using (
                select
                    count(*) tag_count
                from
            ...
            set s.tag_count = t.tag_count;

            merge into eba_ca_tags_sum s
            using (
                select
                    sum(tag_count) tag_count
                from
                    eba_ca_tags_type_sum
                where
                    tag = l_merge_tags(i)
            ) t on ( s.tag = l_merge_tags(i) )
            when not matched then
    ...
    begin
        merge into eba_ca_preferences dest
        using (
            select
                upper(p_preference_name) preference_name,
                p_preference_value       preference_value
            from
                dual
        ) src on ( upper(dest.preference_name) = src.preference_name )
        when matched then update
        set dest.preference_value = src.preference_value
        when not matched then
        insert (
            dest.preference_name,
            dest.preference_value )
        values
            ( src.preference_name,
              src.preference_value );
    ...
    begin
        -- Remove old errors
        delete from eba_ca_errors
        where
            err_time <= current_timestamp - 21;
        -- Log the error.
        insert into eba_ca_errors (
            app_id,
            app_page_id,
            app_user,
            user_agent,
